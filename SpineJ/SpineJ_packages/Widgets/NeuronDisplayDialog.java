...
public class NeuronDisplayDialog extends Frame implements ActionListener {
	...
	protected JButton m_analyzeSpineButton, m_exportShapeSpines, m_flattenButton = null, m_deleteCurrentSpineButton = null, m_exportSpinesTxt = null, m_exportRoisTxt = null, m_exportAllIntensityProfiles = null, m_automaticIdentificationSpines = null, m_loadDataSpines = null;
	...
	public NeuronDisplayDialog( NeuronSegmentor _dss, ImageCanvasNeuron _canvas, Calibration _cal ){
		...
		trianglesRadio.setSelected( true );
		...
		m_exportRoisTxt = new JButton("Export Rois");
		...
		pkActin.pack(m_exportRoisTxt).gridx(0).gridy(4).remainx().fillx().inset(3, 3, 3, 3);
		pkActin.pack( m_exportAllIntensityProfiles ).gridx( 0 ).gridy( 5 ).gridw( 2 ).remainx().fillx().inset( 3, 3, 3, 3 );
		...
		m_exportRoisTxt.setEnabled(true);
		...
		m_lineStepWTF = new JTextField( "" + (calibrationInUm ? 0.05 : 200));//( int )( 20 * m_cal.pixelWidth ) );
		...
		m_exportRoisTxt.addActionListener( this );
	...
	public void actionPerformed( ActionEvent e ) {
		...
		else if(o instanceof JButton)
		{
			...
			//Head ROIs
			...
			else if (o == this.m_exportRoisTxt) {
				String directory = this.m_canvas.getDirectoryOriginalImage();
				if (directory == null) {
				...
				String generalName = this.m_canvas.getImage().getTitle();
				generalName = generalName.substring(0, generalName.lastIndexOf("."));
				//spine
				String nameTxt = generalName + ".txt";
				boolean overwrite = false;
				if ((new File(directory + nameTxt)).exists()) {
					YesNoCancelDialog dialog = new YesNoCancelDialog(this, "Overwrite result file ?", "Do you want to overwrite " + nameTxt);
					...
					ArrayList<NeuronObject> spines = m_canvas.getAnalyzedSpines();
					if (spines == null || spines.isEmpty())
						return;  
					Writer writer = null;
						try { 
							String separator = System.getProperty("line.separator");
							writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(directory + nameTxt), "utf-8"));
							for ( NeuronObject spine : spines ) {
								ArrayList <Point2D.Double> head = spine.getHead();
								...
								writer.write("spine" + separator);
								ArrayList <Point2D.Double> spine_list = new ArrayList<Point2D.Double>();
								for (int i = 0; i < outline.npoints; i++) {
									spine_list.add(new Point2D.Double(outline.xpoints[i],outline.ypoints[i]));
									writer.write("" + outline.xpoints[i] + " " + outline.ypoints[i] + separator);
								}
								
								// head
								writer.write("head" + separator);
								for (Point2D.Double point : head) 
									writer.write("" + (float) point.x + " " + (float) point.y + separator);
								
								// neck
								ArrayList <Point2D.Double> sep = new ArrayList<Point2D.Double>(head);
								sep.removeAll(spine_list);
								ArrayList <Point2D.Double> head_without_sep = new ArrayList<Point2D.Double>(head);
								head_without_sep.removeAll(sep);
								ArrayList <Point2D.Double> neck = new ArrayList<Point2D.Double>(spine_list);
								neck.removeAll(head_without_sep);
								writer.write("neck" + separator);
								for (Point2D.Double point : neck) 
									writer.write("" + (float) point.x + " " + (float) point.y + separator);
									
								// neck skeleton
								writer.write("neck_skeleton" + separator);
								ArrayList<Point2D.Double> skeleton = spine.getSkeletonPath();
								for (Point2D.Double point : skeleton)
									writer.write("" + (float) point.x + " " + (float) point.y + separator);
							} 
						}
